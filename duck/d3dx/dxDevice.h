...
class DxDevice
{
  public:
    ...
    static std::vector<BYTE> LoadByteCode(const std::filesystem::path& filename);
    ...
    template <typename VertexType> dx_ptr<ID3D11InputLayout> CreateInputLayout(const std::vector<BYTE>& vsCode) const
    {
        return CreateInputLayout(VertexType::Layout, vsCode);
    }

    dx_ptr<ID3D11DepthStencilView> CreateDepthStencilView(SIZE size) const;
    template <class T> dx_ptr<ID3D11Buffer> CreateVertexBuffer(const std::vector<T>& vertices) const
    {
    ...
    template <class T> dx_ptr<ID3D11Buffer> CreateVertexBuffer(unsigned int N) const
    {
    ...
    template <typename T> dx_ptr<ID3D11Buffer> CreateIndexBuffer(const std::vector<T>& indices) const
    {
    ...
    template <typename T, size_t N = 1> dx_ptr<ID3D11Buffer> CreateConstantBuffer() const
    {
    ...
    dx_ptr<ID3D11ShaderResourceView> CreateShaderResourceView(const std::filesystem::path& texPath) const;
