...
Viewport::Viewport(SIZE size) : D3D11_VIEWPORT{}
{
    // this->TopLeftX = 0.0f;
    // this->TopLeftY = 0.0f;
...
BufferDescription::BufferDescription(UINT bindFlags, size_t byteWidth) : D3D11_BUFFER_DESC{}
{
    BindFlags = bindFlags;
    ByteWidth = byteWidth;
    // Usage = D3D11_USAGE_DEFAULT;
...
RasterizerDescription::RasterizerDescription()
{
    ...
    FrontCounterClockwise =
        false; // Indicates if vertices of a front facing triangle are counter-clockwise on the render target
    DepthBias             = 0;
    DepthBiasClamp        = 0.0f;
