...
void mini::OrbitCamera::SetTarget(DirectX::XMFLOAT3 v)
{
    m_target = DirectX::XMFLOAT4(v.x, v.y, v.z, 1.f);
}
...
void OrbitCamera::Rotate(float dx, float dy)
{
    ...
    m_angleX = std::clamp(m_angleX, -XM_PI / 2.f, XM_PI / 2.f);
}
...
DirectX::XMFLOAT4 OrbitCamera::getCameraPosition() const
{
    ...
    // auto alt = XMMatrixTranslation(0.0f, 0.0f, -m_distance) * XMMatrixRotationY(m_angleY) *
