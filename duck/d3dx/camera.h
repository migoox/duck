...
{
  public:
    explicit OrbitCamera(DirectX::XMFLOAT3 target = DirectX::XMFLOAT3(0, 0, 0), float minDistance = 0.0f,
                         float maxDistance = FLT_MAX, float distance = 0.0f);
    ...
    {
        MoveTarget(XMLoadFloat3(&v));
    }
    void MoveTarget(DirectX::FXMVECTOR v);
    void SetTarget(DirectX::XMFLOAT3 v);
...
{
  public:
    explicit FPSCamera(DirectX::XMFLOAT3 target) : OrbitCamera(target, 0.0f, 0.0f)
    {
    }
